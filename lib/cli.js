...
module.exports = {
    run: async () => {
        ...
        status = new Spinner('Checking ssh on server side, please wait...');
        status.start();
        try {
            await connection.exec('[ ! -d "~/.ssh" ] && mkdir -p ~/.ssh');
            await connection.exec('test -f ~/.ssh/authorized_keys || touch ~/.ssh/authorized_keys');
            status.stop();
        }catch(e) {
            status.stop();
            await connection.close();
        ...
        status.start();
        try {
            key = await ssh_gen.genKey(hostnameMin);
            status.stop();
        }catch(e) {
            status.stop();
            await connection.close();
